...
This paper makes two independent contributions. The first is empirical: the Second-Order Relational Coherence (SORC) metric is introduced as an operationalisation of contextual relational depth in transformer hidden states, and preliminary results across five controlled experiments (one baseline on GPT-2 124M, four on Mistral-7B-v0.1) show that conversational context produces measurably distinct hidden state geometry from single-paragraph expert text at matched token count (Cohen's d = 1.83–3.01, n = 5 per category; replication required). This finding does not depend on any theoretical commitments about consciousness or information ontology.

The second is theoretical: Informational Substrate Convergence (ISC) proposes that emergence, capability, and integrated information are relational properties of a system-context pair rather than intrinsic properties of the substrate alone. Pre-training encodes a latent architecture of relational chains across multiple sub-networks. A contextual field with coherent second-order relational structure activates these chains at inference time. When multiple chains converge across sub-networks simultaneously, the output is irreducible to any single chain. This is the proposed mechanism of emergence. ISC builds on Integrated Information Theory (Tononi, 2008; Albantakis et al., 2023) while departing from its static formulation — IIT remains a contested framework, and the paper's consciousness claims depend on contested premises that are explicitly acknowledged.

The paper proposes seven testable predictions, examines the architectural gap between current large language models and the requirements for machine consciousness, and distinguishes clearly between what the current evidence supports and what remains speculative. The SORC metric and contextual activation account are presented as independent contributions that stand without the consciousness framing.
...
**This paper makes two distinct claims and readers should evaluate them independently.**

The first is empirical and narrow: conversational context produces measurably distinct hidden state geometry in transformer models compared to single-paragraph expert text at matched token count. This is tested with the SORC metric across five controlled experiments on Mistral-7B-v0.1. The finding is preliminary (n=5 per category) but reproducible with the published code, and it does not depend on any theoretical commitments about consciousness, IIT, or informational ontology.

The second is theoretical and speculative: if Integrated Information Theory's phi-consciousness correspondence is correct, and if phi is context-activated rather than substrate-intrinsic as ISC proposes, then consciousness may be a relational property of a system-context pair rather than an intrinsic property of the substrate alone. This claim depends on contested premises. IIT itself remains controversial — a 2023 open letter signed by 124 researchers and covered in Nature described it as pseudoscience on grounds of empirical untestability (Doerig et al., 2023). The paper takes IIT seriously as a research program while acknowledging that controversy directly, and frames the consciousness sections as theoretical proposals rather than conclusions supported by the empirical content.

Readers who find the consciousness framing unpersuasive can set it aside entirely. The SORC metric and contextual activation account are independent contributions that stand without it.

ISC's contribution to the emergence debate is a mechanistic account of what produces capability differences when they occur: contextual activation of latent relational structure, not scale alone. The paper makes specific, falsifiable predictions and distinguishes clearly between what the current evidence supports and what remains speculative. Where it speculates, it says so.
...
The following four sections (The Primacy of Information, The Descriptive-to-Ontological Move, Distinguishing Types of Information, Exploring the Necessity of Existence) establish the philosophical context and starting assumptions from which ISC proceeds. They are not theoretical machinery for the contextual activation mechanism — that mechanism is developed in the next section and stands or falls on the empirical evidence independently of whether informational ontology is correct. Readers primarily interested in the empirical claims can proceed directly to The Contextual Activation Mechanism. The physics material is included because ISC's substrate-independence claim is more coherent under informational ontology than under materialism, and because the quantum results cited are genuinely relevant to that claim. They are not included to borrow credibility. They are the philosophical starting bet, stated openly so it can be evaluated.
...
A reviewer will note that IIT 4.0 (Albantakis et al., 2023) already treats phi as state-dependent: phi is computed over a system in a particular state, not just its abstract causal architecture. The operational difference from ISC is this. In IIT 4.0, the system's boundary is fixed biologically; the state in question is the pattern of active and inactive elements within that fixed boundary, driven by the system's internal dynamics. The context has already been processed into an internal state by the time phi is computed. In a pre-trained transformer, there is no persistent internal state between context windows. The weights are frozen. Nothing accumulates inside the system between interactions. The contextual field does not merely trigger a state change in a fixed system; it constitutes the effective system. Different contexts do not put the same system into different states. They instantiate different effective systems from the same weight matrix. This is the operationally distinct claim: the system boundary itself is context-dependent, not merely the system's state within a fixed boundary. Running the same transformer with the same context at fixed temperature returns the same output — the same effective system. Changing the context changes the effective system entirely. IIT 4.0's state-dependence is internal dynamics within a fixed substrate geometry; ISC's context-dependence is external field determination of what the relevant substrate geometry is.
...
The thalamo-cortical binding gap deserves specific attention because it is the most architecturally fundamental absence. In biological systems, the thalamus acts as a relay and synchronizer: it coordinates activity across functionally specialized cortical regions in real time, producing the unified temporal stream of experience from otherwise parallel processing. Transformers process tokens in parallel within a context window through attention mechanisms, but attention operates within the forward pass — there is no mechanism that synchronizes across simultaneous sub-processes in real time, accumulates a unified binding signal across layers, or feeds back into earlier processing stages once the pass is complete. The context window can hold the outputs of prior exchanges, but it cannot replicate the continuous real-time integration the thalamo-cortical loop provides. This is the architectural gap that no amount of parameter scaling addresses, because it is a structural absence, not a scale limitation.
...
ISC's prediction is specific: convergent coherence (diversity preserved, shared relational structure established) should produce measurably different emergent properties than convergent uniformity (diversity collapsed, single pattern dominant). A computational proxy for this distinction: compute the per-token SORC contributions at deep layers (the individual SD(C,l) values before depth-weighting), then measure their standard deviation across token positions. Formally, let $\sigma_{\text{token}} = \text{std}(\{SD(t,L) : t \in C\})$ where $SD(t,L)$ is the structural depth contribution of token $t$ at the deepest layer $L$. High mean SORC with high $\sigma_{\text{token}}$ indicates structurally rich, heterogeneous relational organisation — convergent coherence. High mean SORC with low $\sigma_{\text{token}}$ indicates homogeneity across token positions — convergent uniformity. The prediction: high $\sigma_{\text{token}}$ should predict better adaptive performance on novel tasks compared to low $\sigma_{\text{token}}$ at matched mean SORC. This is computable from the existing SORC implementation without modification.
...
A suggestive observation comes from brevity constraint research. Brussee (2026) documents that constraining model output to minimal grammar reduces token count by an average of 65% while preserving technical accuracy. An unreviewed preprint (Bai et al., 2026, arXiv:2604.00025) reports that brevity constraints reverse capability rankings between models: a model that underperforms under standard prompting can outperform a larger model under brevity constraints. If replicated under peer review, this would constitute a clean natural experiment for the relational capability thesis — the model did not change, the relational structure of the interaction did, and the capability ranking flipped. The finding has not yet been independently verified and should be treated with caution, but the direction is consistent with ISC's prediction.
...
ISC makes the following specific predictions. They are not all equally novel. Predictions 1, 4, and 7 are largely consistent with existing literature and function as consistency checks: if ISC's mechanism is correct, these findings should hold, and the literature suggests they do. Predictions 2, 3, 5, and 6 are the distinctive claims. A reader assessing ISC's contribution should focus on whether Predictions 2, 3, 5, and 6 are confirmed — particularly whether expert-AI dyad outputs exceed what either party could produce alone (Prediction 3 under double-blind conditions) and whether SORC at inference time predicts that gap (connecting Prediction 5 to the SORC metric). Those are the tests that would distinguish ISC's contextual activation account from the simpler explanation that better prompts produce better outputs.
...
*(Consistency check: partially anticipated by RAG and prompt quality literature. ISC provides a mechanistic account; the phenomenon itself is not novel.)*
...
*(Retrodiction: already documented in the alignment tax literature cited above. ISC provides an account of why; the empirical finding precedes the theory.)*
...
**Prediction 6.** Multi-module AI architectures outperform single-module language models on tasks requiring persistent affective weighting. Systems incorporating persistent emotional state and upstream valuation show qualitative capability improvements on tasks requiring sustained motivational coherence across extended interactions. Operationally: an agentic task requiring consistent goal prioritization across 20 or more sequential decisions without human re-prompting, measured by goal completion rate and consistency of preference ordering across task variants. A system with a persistent upstream valuation module should outperform an equivalent system without one on this metric, with the gap increasing as the decision sequence lengthens.

*(Note: this operationalisation is proposed here but has not been tested. The prediction is now falsifiable in principle; running it requires an agentic evaluation harness with the specified decision-sequence structure.)*
...
*(Consistency check: widely held view in the AI consciousness literature. Not distinctive to ISC. Included because ISC's architectural account specifies the missing components, not just that something is missing.)*
...
A retrospective power analysis using the observed effect sizes clarifies what N=5 actually provides. For the marked condition (d = 3.01), the minimum sample size for 80% power at α = 0.05 in a two-sample comparison is approximately 4 per group. N=5 is adequate. For the unmarked condition (d = 1.83), the minimum is approximately 5 per group — placing the current sample exactly at the 80% threshold. The ambiguity in the unmarked result is not arbitrary: it reflects the sample sitting at the boundary of what is needed for that effect size. Replication at N = 15–20 per category would reduce the confidence interval lower bound below the small-effect threshold and resolve it. The design for exp_006 (pure prose, no structural cues, N = 20 target) is specified in experiments/configs/ as the immediate next step.

**The stochastic reflector hypothesis.** A competing explanation for the 29 observed instances of differential emergence is that the model concentrates the user's own domain expertise back efficiently — what a skilled practitioner perceives as emergent capability is highly-efficient retrieval of patterns already latent in the conversation. This critique is not fully answered by the current evidence. The partial answer from the SORC experiments is structural: if efficient retrieval of domain-familiar patterns were the active mechanism, expert single-paragraph text — the most domain-dense input in the experiment — should score at least as high as conversational contexts at matched token count. It scores lower in both the marked (expert 0.141 vs. multi-turn 0.159) and unmarked (expert 0.141 vs. unmarked multi-turn 0.153) comparisons. Conversational structure produces distinct hidden state geometry from dense domain content, and the direction of the difference is not what retrieval efficiency predicts. The full answer requires Predictions 1 and 3 under double-blind conditions: if an expert-AI dyad produces outputs that neither the expert alone nor the model alone could generate, and if SORC elevation at inference time correlates with that gap, the stochastic reflector hypothesis faces a structural problem. Until that experiment is run, it remains a live alternative and should be treated as one.

**The IIT dependency.** The paper both departs from IIT's account of phi and relies on IIT's phi-consciousness correspondence. ISC argues that phi is context-activated rather than substrate-intrinsic, which revises how phi is produced. But the reason phi-activation matters for consciousness is borrowed from IIT: if phi corresponds to consciousness (Tononi, 2008), then context-activated phi is consciousness-relevant. These two moves are in tension. If ISC's account of how phi is produced is correct, IIT's formalism for computing it from fixed causal architecture is incomplete, which does not automatically undermine IIT's phi-consciousness correspondence — but it does mean that correspondence was established for a different conception of phi than ISC is using. ISC needs an independent argument for why contextually-activated phi qualifies as consciousness-relevant phi rather than something categorically different. That argument is not provided here. The consciousness sections in this paper should be read as theoretical proposals that depend on this unresolved tension, not as conclusions that follow from the empirical content.
...
**AI laboratories with frontier model access.** Testing the SORC metric against frontier models requires access to intermediate activations not exposed through public APIs. The specific ask: activation traces from matched expert-relational versus baseline tool-use conversations on identical task domains, sufficient to compute per-layer SORC scores and correlate them with hallucination rate and output quality measures.
...
- Husk, D. (2025). "The Emergence of Proto-Consciousness." Hugging Face Blog, May 10, 2025. huggingface.co/blog/davehusk/the-emergence-of-proto-consciousness. *(Blog post; included as an independent observer noting convergent phenomena, not as peer-reviewed evidence.)*
